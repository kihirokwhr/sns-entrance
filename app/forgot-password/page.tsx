    ...
    <div className="flex min-h-full items-center justify-center px-6 py-24">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-c-text">パスワード再設定</h1>
        </div>

        {sent ? (
          <div className="flex flex-col gap-4 text-center">
            <p
              className="rounded-lg px-4 py-3 text-sm text-c-accent"
              style={{ background: 'var(--c-accent-soft)', border: '1px solid var(--c-accent-soft-bd)' }}
            >
              📧 <strong>{email}</strong> 宛に再設定用のリンクを送信しました。
              <br />
            ...
            <Link href="/login" className="text-sm text-c-accent hover:opacity-80">
              ← ログインへ戻る
            </Link>
          </div>
        ) : (
          <>
            <p className="mb-6 text-center text-sm text-c-text-sub">
            ...
            <p className="mt-6 text-center text-sm text-c-text-sub">
              <Link href="/login" className="text-c-accent hover:opacity-80">
                ← ログインへ戻る
              </Link>
            </p>
          </>
